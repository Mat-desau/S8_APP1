...
for trame = 1 : N_trames
    ...
    signal_filtre_LPC(ptr : ptr + L - 1) = trame_OLA;
...
Sw_k = S .* ((abs(S).^Alpha)./abs(S));
Sig_saw = real(ifft(Sw_k));

Swq_k = quant_scal_unif(Sig_saw, min(Sig_saw), max(Sig_saw), 6);

Sw_c = fft(Swq_k);
Sc = Sw_c .* ((abs(Sw_c).^(1/Alpha))./abs(Sw_c));
